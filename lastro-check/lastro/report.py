...
def reconciliation_html(summary: dict[str, object], output_path: str | Path) -> None:
    ...
    rows = "".join(f"<tr><td>{html.escape(str(k))}</td><td>{html.escape(str(v))}</td></tr>" for k, v in sorted(counts.items()))
...
<div class="k"><div class="c"><b>Transactions</b><br>{html.escape(str(summary.get('transactions')))}</div><div class="c"><b>OK</b><br>{html.escape(str(summary.get('ok')))}</div><div class="c"><b>Exceptions</b><br>{html.escape(str(summary.get('exceptions')))}</div></div>
<h2>Financial exposure</h2><p>Potential gap: <b>{html.escape(str(summary.get('potential_financial_gap')))}</b></p>
<p>Unmatched bank entries: <b>{html.escape(str(summary.get('unmatched_bank_entries')))}</b> · total <b>{html.escape(str(summary.get('unmatched_bank_total')))}</b></p>
